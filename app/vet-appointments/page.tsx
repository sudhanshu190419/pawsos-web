...
interface VetProfile {
  ...
  serviceArea: string;
  availability: string | string[]; // e.g. "Mon-Fri 9AM-5PM" or ["Mon", "Tue"]
...
export default function VetAppointmentsPage() {
  ...
  const [consultationTypeFilter, setConsultationTypeFilter] = useState("all"); // 'all', 'clinic', 'remote', 'emergency'
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(q, (snap) => {
      ...
      // Filter out inactive vets if active status field exists
  ...
  const filteredVets = useMemo(() => {
    return vets.filter(v => {
      // 1. Search Query (name, clinic name, service area)
      ...
      // 2. City Filter
      if (cityFilter !== "all" && v.city?.toLowerCase() !== cityFilter.toLowerCase()) {
        return false;
      }
    ...
    try {
      ...
      const response = await fetch("/api/razorpay/create-order", {
        ...
        body: JSON.stringify({
          ...
          amount: bookingAmount * 100, // back-up value, server recalculates
          currency: "INR"
        })
      });
  ...
  return (
    <div className="min-h-screen bg-[#FAFAFA] pb-24 text-slate-800">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-orange-50 via-amber-50/50 to-white pt-8 pb-10 md:pt-14 md:pb-16 px-4 border-b border-slate-100">
        ...
        <div className="max-w-6xl mx-auto relative z-10">
          ...
          <div className="max-w-2xl mx-auto bg-white p-3 rounded-2xl md:rounded-3xl shadow-xl shadow-orange-900/5 border border-slate-100 flex flex-col md:flex-row gap-2.5">
            <div className="flex-1 flex items-center gap-2 px-3 py-1 bg-slate-50/50 rounded-xl">
              ...
              <input
                ...
                className="w-full bg-transparent outline-none font-medium placeholder:font-normal text-slate-800 text-sm py-2"
              />
            </div>
            <div className="flex flex-row gap-2 shrink-0">
              <select
                ...
                className="flex-1 md:flex-initial bg-slate-50 border-0 rounded-xl px-4 py-3 outline-none font-bold text-xs text-slate-700 cursor-pointer min-w-[120px]"
              >
      ...
      <div className="max-w-6xl mx-auto px-4 mt-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          {/* Side Filters Panel */}
          <aside className="lg:col-span-1 bg-white rounded-3xl p-6 border border-slate-100 shadow-md h-fit lg:sticky lg:top-24">
            ...
            <div className="space-y-6">
              ...
              {/* Consultation Type */}
              <div>
                <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-2.5">Consultation Type</label>
                ...
                <div className="flex items-center gap-1.5">
                  {[0, 3, 4, 4.5].map((stars) => (
                    <button
                      ...
                      className={`flex-1 py-2 rounded-xl border text-[11px] font-bold flex items-center justify-center gap-1 transition-all ${
                        minRating === stars
          ...
          <main className="lg:col-span-3">
            {loadingVets ? (
              ...
              <div className="bg-red-50 text-red-700 p-5 rounded-2xl flex items-start gap-3 border border-red-100 font-medium text-sm">
                <AlertTriangle className="w-5 h-5 shrink-0 mt-0.5" />
                <p>{errorMessage}</p>
              </div>
            ) : filteredVets.length === 0 ? (
              <div className="bg-white rounded-3xl p-10 md:p-16 border border-slate-100 shadow-md text-center">
                <div className="w-20 h-20 rounded-full bg-slate-50 flex items-center justify-center mx-auto mb-6">
                  <Search className="w-8 h-8 text-slate-300" />
                </div>
                <h3 className="text-xl font-bold text-slate-800 mb-2">No verified Vets found</h3>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {filteredVets.map((vet) => {
                  ...
                  return (
                    <div
                      key={vet.uid}
                      className="bg-white rounded-3xl border border-slate-100 shadow-sm hover:shadow-xl hover:-translate-y-1 transition-all duration-300 overflow-hidden flex flex-col h-full"
                    >
                      {/* Top Header Card Info */}
                      <div className="p-6 flex-1">
                        <div className="flex gap-4 items-start">
                          <img
                            src={vet.profilePhotoURL || "https://ui-avatars.com/api/?name=Doctor&background=fff4e6&color=ea580c&size=128"}
                            alt={vet.fullName}
                            className="w-16 h-16 rounded-2xl border object-cover shadow-inner bg-slate-50 shrink-0"
                          />
                          <div className="min-w-0">
                            <div className="flex items-center gap-1.5 flex-wrap mb-1">
                              <span className="bg-blue-50 text-blue-600 font-extrabold text-[9px] px-2 py-0.5 rounded-full uppercase tracking-wider flex items-center gap-0.5">
                                <ShieldCheck className="w-3 h-3" /> Verified
                              </span>
                              {vet.willingToTravel && (
                                <span className="bg-emerald-50 text-emerald-600 font-extrabold text-[9px] px-2 py-0.5 rounded-full uppercase tracking-wider">
                                  🚗 House Calls
                                </span>
                              )}
                            </div>
                            <h4 className="text-base font-extrabold text-slate-800 truncate leading-snug">
                              Dr. {vet.fullName}
                            </h4>
                        ...
                        {/* Location Details */}
                        <div className="mt-4 space-y-2 border-t border-slate-50 pt-4 text-xs font-semibold text-slate-600">
                        ...
                        {/* Ratings & Price Bar */}
                        <div className="mt-5 bg-slate-50/50 rounded-2xl px-4 py-3 flex justify-between items-center">
                          <div className="flex items-center gap-1">
                            <Star className={`w-4 h-4 ${averageRating > 0 ? "fill-orange-500 text-orange-500" : "text-slate-300"}`} />
                            <span className="text-xs font-black text-slate-800">{averageRating > 0 ? averageRating.toFixed(1) : "New"}</span>
                            <span className="text-[10px] text-slate-400 font-bold">({reviewCount})</span>
                          </div>
                          <div className="text-right">
                            <p className="text-[9px] text-slate-400 font-black uppercase">Cons. Fee</p>
                      ...
                      <div className="p-6 pt-0 border-t border-slate-50 bg-white">
                        <button
                          ...
                          disabled={!hasPricing}
                          className={`w-full py-3.5 rounded-2xl font-bold text-xs flex items-center justify-center gap-2 transition-all active:scale-95 shadow-md ${
                            hasPricing
      ...
      {selectedVet && (
        <div className="fixed inset-0 flex items-end sm:items-center justify-center p-0 sm:p-4 z-[99999]">
          ...
          <div className="bg-white rounded-t-3xl sm:rounded-3xl w-full max-w-lg p-6 sm:p-8 shadow-2xl relative z-[100000] max-h-[90vh] overflow-y-auto animate-in slide-in-from-bottom duration-200">
            
            {/* Modal Header */}
            <div className="flex justify-between items-center mb-6">
              <div>
                <h3 className="text-xl sm:text-2xl font-black text-slate-800 leading-snug">Book Appointment</h3>
                <p className="text-slate-500 text-xs font-semibold mt-0.5">Dr. {selectedVet.fullName} • {selectedVet.clinicName || "Vet clinic"}</p>
              </div>
              <button 
            ...
            {bookingSuccess ? (
              ...
              <form onSubmit={handlePayAndBook} className="space-y-4">
                ...
                <div>
                  ...
                  {pets.length > 0 ? (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mb-2">
                      {pets.map(p => (
                        ...
                        >
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-slate-400 capitalize">{p.breed}</p>
                          </div>
                        </button>
                      ))}
                      <button
                      ...
                      >
                        ...
                        <div>
                          <p className="text-xs font-extrabold">Enter custom pet</p>
                          <p className="text-[10px] text-slate-400">Not registered in app</p>
                        </div>
                      </button>
                    </div>
                  ) : (
                  ...
                  {bookingPetId === "manual" && (
                    <div className="grid grid-cols-2 gap-3 bg-slate-50 p-3.5 rounded-2xl border mt-2">
                      <div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="w-full bg-slate-50 border rounded-xl px-4 py-2.5 text-xs outline-none focus:border-orange-500 font-bold text-slate-800 cursor-pointer"
                    >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    className="w-full bg-slate-50 border rounded-xl px-4 py-2.5 text-xs outline-none focus:border-orange-500 font-bold text-slate-800 placeholder:font-normal"
                  />
                </div>
                ...
                {bookingError && (
                  <div className="bg-red-50 text-red-700 p-3.5 rounded-xl border border-red-100 flex items-start gap-2 text-xs font-bold">
                ...
                <div className="flex gap-3 pt-3">
                  <button
                    type="button"
                    onClick={closeBookingModal}
                    className="flex-1 py-3.5 rounded-2xl font-bold text-slate-500 hover:bg-slate-50 active:scale-95 transition-all text-xs"
                  >
                    Cancel
                  </button>
                  <button
                    ...
                    className="flex-[2] bg-orange-500 hover:bg-orange-600 text-white font-bold py-3.5 rounded-2xl text-xs active:scale-95 transition-all disabled:bg-slate-300 shadow-md shadow-orange-500/10 flex items-center justify-center gap-2"
                  >
                    {isSubmittingBooking ? (
                      <>
